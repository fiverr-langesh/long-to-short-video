...
def segment_video(response,input_path,output_path):
  for i, segment in enumerate(response):
        ...
        output_file = f"{output_path}/output{str(i).zfill(3)}.mp4"
        command = f"ffmpeg -i {input_path} -ss {start_time} -to {end_time} -c copy {output_file}"
...
def main(inp):
    url = inp["url"] # Replace with your video's URL
    userId = inp["user_id"]

    video_id=url.split("=")[1]
    filename = 'input_video.mp4'
    input_path = f'uploads/{userId}/inputs/{filename}'
    output_path = f'uploads/{userId}/outputs'
...
# Sample json
# {
#   url: "https://www.youtube.com/watch?v=92nse3cvG_Y",
#   user_id : 1234
# }
