...
export default function RecentActivity() {
  const { recent ,credit, processingTime} = useContext(RecentActivityContext);
  const [language, setLanguage] = React.useState("english");

  if (!recent.length) return; 

  return (
    <div className=" my-10 space-y-6 flex flex-col items-center justify-center">
      <div className=" flex items-center gap-16">
        <div className=" flex items-center gap-4">
          <h1 className=" text-stone-400  font-semibold text-lg">Caption: </h1>
          <FormControl
            variant="standard"
        ...
        <div className=" flex items-center">
          ...
          <p className=" ml-1  text-stone-200 font-medium">~ {credit.toFixed(2)}m</p>
        </div>
      </div>
